...
@dataclass
class BaseModelOutput_LST(ModelOutput):
    last_hidden_state: torch.FloatTensor = None
    hidden_states: Optional[Tuple[torch.FloatTensor, ...]] = None
    attentions: Optional[Tuple[torch.FloatTensor, ...]] = None
    LST_states: torch.FloatTensor = None
...
class WhisperEncoderLayer(nn.Module):
    def __init__(self, config: WhisperConfig):
        ...
        # -----------------------------------------------------------------------
        self.reduction_factor_LST = 8
        self.LST_embed_dim =  self.embed_dim // self.reduction_factor_LST

        self.side_downsample = nn.Sequential(nn.LayerNorm(self.embed_dim), nn.Linear(self.embed_dim, self.LST_embed_dim)) 

        self.LST_block = AdapterModule(self.LST_embed_dim, 256)
    ...
    def forward(
        ...
        hidden_states: torch.Tensor,
        # -----------------------------------------------------------------------
        previous_LST_states: torch.Tensor,
        # -----------------------------------------------------------------------
        ...
        u = torch.sigmoid(self.gate_param / 0.1)
        LST_states = u * previous_LST_states + (1 - u) * self.side_downsample(hidden_states)
        LST_states = self.LST_block(LST_states)
        # -----------------------------------------------------------------------
        ...
        if LST_states.dtype == torch.float16 and (
            torch.isinf(LST_states).any() or torch.isnan(LST_states).any()
        ):
            clamp_value = torch.finfo(LST_states.dtype).max - 1000
            LST_states = torch.clamp(LST_states, min=-clamp_value, max=clamp_value)
...
class WhisperEncoder(WhisperPreTrainedModel):
    ...
    def __init__(self, config: WhisperConfig):
        ...
        reduction_factor_LST = 8
        LST_embed_dim = embed_dim // reduction_factor_LST
        ...
        # -----------------------------------------------------------------------
        self.first_downsample = nn.Sequential(nn.LayerNorm(embed_dim),  # 先归一化
                                                      nn.Linear(embed_dim, LST_embed_dim)) 

        self.final_upsample = nn.Sequential(nn.LayerNorm(LST_embed_dim),  # 先归一化
                                                      nn.Linear(LST_embed_dim, embed_dim)) 
        self.layer_norm_LST = nn.LayerNorm(config.d_model)
        # -----------------------------------------------------------------------
    ...
    def forward(
        ...
        # -----------------------------------------------------------------------
        # hidden_states = hidden_states.detach()
        LST_states = self.first_downsample(hidden_states)
        # -----------------------------------------------------------------------
            ...
            if to_drop:
                ...
                if self.gradient_checkpointing and self.training:
                    layer_outputs = self._gradient_checkpointing_func(
                        ...
                        output_attentions,
                        previous_LST_states = LST_states,
                    )
                else:
        ...
        # -----------------------------------------------------------------------
        LST_states = self.final_upsample(LST_states)
        LST_states = self.layer_norm_LST(LST_states)
        # -----------------------------------------------------------------------
        ...
        return BaseModelOutput_LST(
            last_hidden_state=hidden_states, hidden_states=encoder_states, attentions=all_attentions, LST_states=LST_states
        )
...
class WhisperForAudioClassification(WhisperPreTrainedModel):
    ...
    def forward(
        ...
        # -----------------------------------------------------------------------
        LST_states = encoder_outputs[-1]
        hidden_states = LST_states
        # -----------------------------------------------------------------------
