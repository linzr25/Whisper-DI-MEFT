...
class AdapterModule(nn.Module):
    def __init__(
        self,
        input_dim: int,
        hidden_dim: int,  # 如果未提供 hidden_dim，则根据 reduction_factor 计算
    ):
        """
        初始化 AdapterModule。
        ...
        Args:
            input_dim (int): 输入维度。
            hidden_dim (int, optional): 隐藏层维度。如果未提供，将使用 input_dim // reduction_factor 计算。
            reduction_factor (int, optional): 降维因子，用于计算 hidden_dim。默认为32。
        """
        super().__init__()
...
class WhisperEncoderLayer(nn.Module):
    def __init__(self, config: WhisperConfig):
        ...
        # self.adapter = AdapterModule(self.embed_dim,128)
        # self.adapter2 = AdapterModule(self.embed_dim,128)
...
class WhisperEncoder(WhisperPreTrainedModel):
    ...
    def __init__(self, config: WhisperConfig):
        ...
        self.side_block = Sherl_Block(embed_dim, 
                                      embed_dim // reduction_factor,
                                      config.encoder_layers)

        self.layers_num =config.encoder_layers
    ...
    def forward(
        ...
        # //////////////////////////////////////////////////
        all_hidden_features = []
        all_hidden_features.append(hidden_states)
            ...
            if to_drop:
                ...
                if idx < self.layers_num - 1:
                    ...
                    if self.gradient_checkpointing and self.training:
                        ...
                        hidden_states = self.side_block(all_hidden_features, hidden_states)
                        layer_outputs = encoder_layer(
                            hidden_states,
                            None,
                            layer_head_mask=(head_mask[idx] if head_mask is not None else None),
