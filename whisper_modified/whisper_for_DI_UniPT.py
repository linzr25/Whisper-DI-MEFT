...
class WhisperEncoder(WhisperPreTrainedModel):
    ...
    def __init__(self, config: WhisperConfig):
        ...
        d_model = embed_dim
        reduction_factor = 16
        self.d_side = embed_dim // reduction_factor

        self.side_downsample0_encoder = nn.Sequential(nn.LayerNorm(d_model),  # 先归一化
                                                      nn.Linear(d_model, self.d_side)) 
        
        self.side_downsamples_encoder = nn.ModuleList([nn.Sequential(nn.LayerNorm(d_model),  # 先层归一化
                                                                     nn.Linear(d_model, self.d_side))
                                                                     for _ in range(config.encoder_layers)])
        
        self.side_projection_encoder = nn.Sequential(nn.LayerNorm(self.d_side),
                                                     nn.Linear(self.d_side, d_model),
                                                     nn.GELU(),
                                                     nn.Linear(d_model, self.d_side))
        self.side_gate_layer_encoder = nn.Linear(self.d_side, 1)

        self.side_upsample = nn.Sequential(nn.LayerNorm(self.d_side),
                                                 nn.ReLU(),
                                                 nn.Linear(self.d_side, d_model))
                                                # nn.LayerNorm(d_model))

        self.layer_norm_unipt_encoder = nn.LayerNorm(d_model)
        # -----------------------------------------------------------------------
    ...
    def cross_attention(self, query, context, mask=None, residual=True, **kwargs):
        ...
        cross_weights = torch.bmm(query, context.transpose(1, 2))
    ...
    def forward(
        ...
        # -----------------------------------------------------------------------
        hidden_states = hidden_states.detach()
        all_downsample_features_encoder = []
        all_downsample_features_encoder.append(self.side_downsample0_encoder(hidden_states))
        # -----------------------------------------------------------------------
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if to_drop:
                ...
                # -----------------------------------------------------------------------
                hidden_states = hidden_states.detach()
                side_states_encoder = self.side_downsamples_encoder[idx](hidden_states)
                all_downsample_features_encoder.append(side_states_encoder)
                # -----------------------------------------------------------------------
        ...
        # -----------------------------------------------------------------------
        assert len(all_downsample_features_encoder) == len(self.layers) + 1 

        confidence_set = []
        merged_features_set = []
        
        anchor_features = all_downsample_features_encoder[-1]
        anchor = self.aggregate_feature(anchor_features) ## No Mask
            
        for index in range(len(self.layers)):

            target_features = all_downsample_features_encoder[index]
            target = self.aggregate_feature(target_features)  ## No Mask
            merged_features, _ = self.cross_attention(anchor_features, target_features)  ## No Mask
            merged_features_set.append(merged_features) ## M i = L1NormF i σ(F N F ⊤  i )+I, F′  i = M iF i , (1)
            confidence_set.append(self.side_gate_layer_encoder(anchor * target)) ## FgN ⊙ Fg\N
        
        confidence_norm = torch.softmax(torch.cat(confidence_set, dim=1), dim=1)  ## C = SoftmaxF g  \N ((F g  N ⊙Fg  \N )W 3) ,
        all_merged_features = torch.stack(merged_features_set, dim=1)
        merged_features = torch.sum(all_merged_features * confidence_norm[:, :, None, None], dim=1) ## Ci · F ′  i
        confidence_output_encoder = anchor_features + self.side_projection_encoder(merged_features) ## F c = MLP(  X  i Ci · F ′  i) + F N .
        # -----------------------------------------------------------------------
        
        # hidden_states = self.layer_norm(hidden_states)
        # -----------------------------------------------------------------------
        confidence_output_encoder = self.side_upsample(confidence_output_encoder)
        hidden_states = self.layer_norm_unipt_encoder(confidence_output_encoder)
...
class WhisperDecoder(WhisperPreTrainedModel):
    ...
    def __init__(self, config: WhisperConfig):
        ...
        self.side_downsample0_decoder = nn.Sequential(nn.LayerNorm(d_model),  # 先归一化
                                                      nn.Linear(d_model, self.d_side)) 
        
        self.side_downsamples_decoder = nn.ModuleList([nn.Sequential(nn.LayerNorm(d_model),  # 先归一化
                                                                     nn.Linear(d_model, self.d_side))
                                                                     for _ in range(config.encoder_layers)])
        
        self.side_projection_decoder = nn.Sequential(nn.LayerNorm(self.d_side),
                                                     nn.Linear(self.d_side, d_model),
                                                     nn.GELU(),
                                                     nn.Linear(d_model, self.d_side))
        self.side_gate_layer_decoder = nn.Linear(self.d_side, 1)

        self.side_projection_crosser = nn.ModuleList([nn.Sequential(nn.LayerNorm(self.d_side),# 先归一化
                                                                    nn.Linear(self.d_side, self.d_side))
                                                      for _ in range(config.encoder_layers)])
        
        self.side_final_upsample = nn.Sequential(nn.LayerNorm(self.d_side),
                                                #  nn.ReLU(),
                                                 nn.Linear(self.d_side, d_model),
                                                 nn.LayerNorm(d_model))
        
        # self.layer_norm_unipt_decoder = nn.LayerNorm(config.d_model)
        # -----------------------------------------------------------------------
    ...
    def forward(
        ...
        return_dict=None,
        cache_position=None,
        confidence_output_encoder=None,
    ):
        r"""
        Args:
        ...
        # -----------------------------------------------------------------------
        # hidden_states = hidden_states.detach()
        all_downsample_features_decoder = []
        all_downsample_features_decoder.append(self.side_downsample0_decoder(hidden_states))
        # -----------------------------------------------------------------------
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            # -----------------------------------------------------------------------
            # hidden_states = hidden_states.detach()
            side_states_decoder = self.side_downsamples_decoder[idx](hidden_states)
            all_downsample_features_decoder.append(side_states_decoder)
            # -----------------------------------------------------------------------
        ...
        # -----------------------------------------------------------------------
        assert len(all_downsample_features_decoder) == len(self.layers) + 1 

        confidence_set = []
        merged_features_set = []
        layer_features_set = []
        
        anchor_features = all_downsample_features_decoder[-1]
        anchor = self.aggregate_feature(anchor_features) ## No Mask
            
        for index in range(len(self.layers)):

            target_features = all_downsample_features_decoder[index]
            # 进行交叉注意力
            related_memory, _ = self.cross_attention(target_features, 
                                                     confidence_output_encoder, 
                                                     residual=False)
            # 加上投影后的相关记忆
            target_features = target_features + self.side_projection_crosser[index](related_memory)
        ...
        hs = self.side_final_upsample(side_tgt_memory)
        ...
        # -----------------------------------------------------------------------
        # hidden_states = hidden_states.detach()
        # print(f"The hidden_states of hs is: {hidden_states.shape}")
        # -----------------------------------------------------------------------
...
# -----------------------------------------------------------------------
@dataclass
class BaseModelOutput_UniPT(ModelOutput):
    last_hidden_state: torch.FloatTensor = None
    confidence_output_encoder: torch.FloatTensor = None
...
class WhisperModel(WhisperPreTrainedModel):
    ...
    def forward(
        ...
        # -----------------------------------------------------------------------
        # Mask input features along time axis and/or feature axis   
        ...
        # If the user passed a tuple for encoder_outputs, we wrap it in a BaseModelOutput when return_dict=True
            ...
            encoder_outputs = BaseModelOutput_UniPT(
                last_hidden_state=encoder_outputs[0],
                confidence_output_encoder=encoder_outputs[1],
        ...
        decoder_outputs, hs = self.decoder(
